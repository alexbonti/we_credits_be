...
const createProductTypes = {
  ...
  handler: function (request, h) {
    ...
    return new Promise((resolve, reject) => {
      Controller.AdminBaseController.createProductTypes(
        userData,
        payloadData,
        (err, data) => {
  ...
  options: {
    ...
    validate: {
      payload: Joi.object({
        name: Joi.string().required().valid(...Object.values(Config.APP_CONSTANTS.DATABASE.PRODUCT_TYPES)),
        description: Joi.string().required()
      }).label("Admin: Create Product Types"),
      failAction: UniversalFunctions.failActionFunction,
    },
    plugins: {
      "hapi-swagger": {
...
const getProductTypes = {
  ...
  handler: function (request, h) {
    const userData =
      ...
      null;
    return new Promise((resolve, reject) => {
      Controller.AdminBaseController.getProductTypes(
        userData,
        (err, data) => {
          if (!err) {
            resolve(UniversalFunctions.sendSuccess(undefined, data));
...
const getProductApprovalRequests = {
  method: "GET",
  path: "/api/admin/products/requests",
  options: {
    description: "get Product Approval Requests",
    auth: "UserAuth",
    tags: ["api", "admin"],
    handler: (request) => {
      const userData = (request.auth && request.auth.credentials && request.auth.credentials.userData) || null;
      return new Promise((resolve, reject) => {
        Controller.AdminBaseController.getProductApprovalRequests(userData,request.query, (error, success) => {
          if (error) {
            reject(UniversalFunctions.sendError(error));
          } else {
            resolve(
...
const approveProduct = {
  ...
  options: {
    ...
    handler: (request) => {
      const userData = (request.auth && request.auth.credentials && request.auth.credentials.userData) || null;
      return new Promise((resolve, reject) => {
        Controller.AdminBaseController.approveProduct(userData,request.params, (error, success) => {
          if (error) {
            reject(UniversalFunctions.sendError(error));
          } else {
            resolve(
...
const getProductDetails = {
  ...
  options: {
    ...
    handler: (request) => {
      const userData = (request.auth && request.auth.credentials && request.auth.credentials.userData) || null;
      return new Promise((resolve, reject) => {
        Controller.AdminBaseController.getProductDetails(userData,request.params, (error, success) => {
          if (error) {
            reject(UniversalFunctions.sendError(error));
          } else {
            resolve(
